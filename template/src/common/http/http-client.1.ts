...
export default class HttpClient
{
    ...
    private getAxiosRequest(method: string, request: IHttpRequest): AxiosRequestConfig
    {
        ...
        {
            config.headers["Authorization"] = "Credential " + ApplicationContext.current.credential.credentialId;
        }
        else
        { */
        const token = localStorage.getItem("token");
        token && (config.headers["Authorization"] = "Credential " + token);
        // }
    ...
    private async onRequestComplete(axiosResponse: AxiosResponse | any, axiosRequest: AxiosRequestConfig): Promise<any>
    {
        ...
        else
        {
            ...
            if(code === HttpResponseCode.failure)
            {
    ...
    private resolveResponse(axiosResponse: AxiosResponse, axiosRequest: AxiosRequestConfig): IHttpResponse
    {
        ...
        {
            // 根据扩展标识前缀进行完全匹配（区分大小写）
            const extrasSymbol = extrasSymbols.find(s => s === key.substr(0, s.length));

            if(extrasSymbol)
            {
                // 取扩展标识后的字符串，并且改为小驼峰作为扩展属性名字
                // 例：x-paging-size 解析后为 pagingSize
                const extraName = camelcase(key.substr(extrasSymbol.length));

                extras[extraName] = headers[key];
            }
        });
